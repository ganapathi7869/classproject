...
class RequestGenerator(Thread):
    ...
    def detail_college(self, college_id):
        url = "http://127.0.0.1:8080/api/v1/colleges/" + str(college_id) + "/"
        response = requests.get(url, auth=HTTPBasicAuth('ganapathi7869', 'kgp@1998'))
    ...
    def run(self):
        ...
        try:
            ...
            if len(colleges) > 0:
                college_id = randint(2, 23)   #college_id = randint(1, len(colleges))
                college_detail = self.detail_college(college_id)
                print(college_detail,college_id)
            end = time.time()
